...
1. **Harbor**: 早期分布式方案（Master/Slave 架构），跨进程/跨机器，通过 TCP 建链，提供全局名字同步与透明路由，适合小中规模集群
2. **Cluster**: 现代集群方案（Lua 层 cluster 模块），跨进程/跨机器，按配置映射服务名与节点，适合大规模分布式系统
...
| 特性 | Harbor | Cluster |
|------|--------|---------|
| 部署模式 | 多进程/跨机器 | 多进程/跨机器 |
| 节点数限制 | 最多255个（Harbor ID） | 理论不限制（取决于配置） |
| 通信方式 | TCP Socket（C 层 harbor） | TCP Socket（Lua 层 cluster） |
| 服务发现 | 全局名字自动同步（Master） | 通过配置（clustername） |
| 容错性 | Master 中心，节点断连广播 | 名称解析与 sender 重连 |
| 适用场景 | 小中型集群、透明远程 | 大规模按服务映射的 RPC |
...
Harbor 是 Skynet 早期的分布式方案，所有节点为独立进程（可跨机器），通过 Master/Slave 架构建立 TCP 连接：
...
-- 判断是否远程地址（不同 harbor id 即为远程）
local function is_remote(addr)
  local harbor_id = skynet.harbor(addr)
  return harbor_id ~= 0 and harbor_id ~= skynet.harbor(skynet.self())
end
```
...
-- 在 Harbor 2 通过“全局名字”访问（底层经 .cslave/master 解析到远端 handle）
skynet.start(function()
    -- 可直接通过名字调用（系统将解析为远端地址）
...
local function monitor_harbor_status()
    ...
    -- 等待与指定 harbor 建立连接：
    --  - .cslave.LINK 会在连接事件发生时唤醒；若已连通将立即返回
...
local function monitor_master_status()
    -- 等待与 master 的连接事件；若 master 断连也会唤醒
